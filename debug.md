...
## 新发现的问题
- **记录级随机划分可能直接导致少数类缺失**：即使训练中使用了加权采样、logit 校正和类权重，如果验证/泛化记录里囊括了唯一或大部分的 S/V/O 样本，训练集依旧可能完全缺少这些类别，导致模型召回接近 0，出现“全判 N”或严重偏斜的预测。

## 本次修改
- **记录级分层划分**：增加 `count_record_labels` 与基于记录标签计数的分层函数 `split_train_val_records`，在划分时读取每个记录的注释并统计可截取的四类样本数，随机尝试 2000 次寻找同时保证 Train 和 Val 都覆盖四类的划分，否则直接报错提示调整 `val_ratio` 或 `seed`。这样可避免训练集缺类导致的坍塌预测。
