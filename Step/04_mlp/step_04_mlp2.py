...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Step 04: export MLP2 output.")
...
def main() -> None:
    ...
    np.savez(os.path.join(args.output_dir, "input_04_2.npz"), tokens_matrix=tokens_matrix, label_text=label_text)
    ...
    with quantized_weights(model, bits=args.weight_bits):
        for layer in list(model.mlp_layers)[:2]:
            h = model._scale_if_needed(h)
            h = layer(h)
            h = activation(h)
        weights = list(model.mlp_layers)[:2][1].weight.detach().cpu().numpy()
        write_mlp_csv(
            "02_mlp_2.csv",
            h,
            args.output_dir,
    ...
    np.savez(
        os.path.join(args.output_dir, "output_04_2.npz"),
        mlp2_output=h.detach().cpu().numpy(),
