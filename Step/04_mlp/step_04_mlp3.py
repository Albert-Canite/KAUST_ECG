...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Step 04: export MLP3 output.")
...
def main() -> None:
    ...
    np.savez(os.path.join(args.output_dir, "input_04_3.npz"), tokens_matrix=tokens_matrix, label_text=label_text)
    ...
    with quantized_weights(model, bits=args.weight_bits):
        for layer in list(model.mlp_layers)[:3]:
            h = model._scale_if_needed(h)
            h = layer(h)
            h = activation(h)
        weights = list(model.mlp_layers)[:3][2].weight.detach().cpu().numpy()
        write_mlp_csv(
            "03_mlp_3.csv",
            h,
            args.output_dir,
    ...
    np.savez(
        os.path.join(args.output_dir, "output_04_3.npz"),
        mlp3_output=h.detach().cpu().numpy(),
