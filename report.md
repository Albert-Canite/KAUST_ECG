...
## v2.1debug 复盘与建议（最新日志：纯 CE 基线泛化 miss≈30%）
- **指标解读**：F1 ∈ [0,1]，0.5 附近意味着查准查全均衡但偏低；miss≈30% 说明召回仍不足，尤其在泛化集上漏检多。
- **日志复盘**：
  - Val F1 在 0.42–0.50 间震荡，对应阈值多在 0.45–0.55；Gen F1 随阈值漂移在 0.30→0.57，miss 从 75% 下降到 31–50%，整体仍偏高。
  - Val/Gen 阈值差异导致泛化召回不稳：如第 20–23 轮阈值降到 0.43–0.50，Gen miss 短暂下降到 28–37%，随后阈值回升又出现 45%+ 漏检。
  - 训练 loss 逐步下降且未崩溃，说明模型已学到区分度，但输出概率校准与类别不平衡仍影响阈值泛化。
- **原因推测**：
  1. **类别比例不匹配**：Val/Gen 异常占比可能不同，导致按 Val 扫描得到的阈值迁移到 Gen 时召回下滑。
  2. **容量/正则限制**：学生仅 4 通道 token + 3 层 MLP，配合 0.2 dropout，可能不足以覆盖泛化集中更大形态差异。
  3. **阈值目标单一**：当前 sweep 仅以 F1 最大化，未对 miss 设置上限，可能在 Val 上选择偏高阈值以换取 FPR，而牺牲泛化召回。
- **改进建议（保持 CE 纯净前提下）**：
  - **阈值约束**：在 `sweep_thresholds` 中加入 miss 上限（如 20–25%）或以 `miss_weighted_f1` 方式偏向召回，再观察 Gen miss 是否随之下降。
  - **容量微调**：尝试 `--num_mlp_layers 4` 或降低 `--dropout_rate` 至 0.1，提升特征表达；必要时增加 token 通道宽度到 8（需改 Student 模型）。
  - **数据平衡轻量化**：保留当前类权重外，可在 DataLoader 层做简单过采样（仅异常类倍数 1.2–1.5），避免训练分布过度偏向正常类。
- **运行日志记录方式**：训练时自动写入 `artifacts/training_log_YYYYMMDD_HHMMSS.jsonl`，记录起始配置、每轮阈值/指标、best 检查点及最终指标，便于后续直接读取。
