...
- **本轮改动（结合最新日志的参数再调优）**：
  ...
  - **最新补充：无收益自动关闭 KD**：新增 `--kd_disable_patience` 与 `--kd_disable_delta`，在 KD 启动后若验证 F1 连续多轮未超过“无 KD 阶段最佳 F1+delta”，则自动关闭 KD 回退到纯 CE，并记录关闭事件，防止强行蒸馏拖慢或恶化收敛。【F:train.py†L79-L178】【F:train.py†L516-L556】
- **后续建议**：
