...
- **现状解读**：
  ...
  3. **漏诊/FPR 已低于目标**：miss 与 FPR 均满足 15% 上限，当前瓶颈主要在验证侧精度（FP）与少量 FN，可在不破坏泛化的前提下微调以提升验证 F1。
- **优化建议（保持无 KD 与现有容量）**：
...
- **异常类强化但仍限幅**：默认 `class_weight_abnormal` 提升至 **1.40**，保持比值上限 2.0；异常过采样倍率暴露为 `--abnormal_sampler_boost`（默认 1.2），并允许通过 `--force_balanced_batches` 在异常占比不足时强制 1:1 batch 采样。
- **阈值搜索更偏向低 FPR 且靠近经验最优**：`sweep_thresholds_blended` 增加 FPR 惩罚系数与阈值正则（默认中心 0.80、强度 0.03），得分公式调整为 `f1 + 1.5*TPR - penalty*FPR - reg*|thr-center|`，在 miss/FPR 约束内优先选择低 FPR 且接近已验证有效区间的阈值。
- **可控实验开关**：新增 `--threshold_fpr_penalty`、`--threshold_center`、`--threshold_reg` 与 `--force_balanced_batches`，便于在不改模型容量的前提下对比不同阈值偏好和批次平衡策略对验证 FPR/总体 F1 的影响。
