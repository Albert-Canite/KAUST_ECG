...
- **Photonic MLP**：`num_mlp_layers`（默认 1，最少 1，可显式设为 1 层）层 `Linear(4,4)+ReLU/tanh`，逐 token 独立映射，保持 `(batch, 8, 4)`。
...
## 学生模型结构确认（代码现状）
- **卷积编码**：四个分段 `Conv1d(1, 4, kernel_size=4, stride=1, padding=0)`，对应 P/QRS/T/全局片段，各自输出 4 个通道。
- **Token 池化**：P 段 2 个 token、QRS 段 3 个 token、T 段 2 个 token，全局段 1 个 token，拼成 `(batch, 8, 4)` 的 token 序列。
- **MLP 深度**：`num_mlp_layers` 现在允许最少 1 层（默认 1 层），每层是 `Linear(4,4)+ReLU/tanh`，逐 token 作用，不再强制至少 2 层。
- **分类头**：token 均值池化得到 `(batch, 4)` 的全局 embedding，经可选 Dropout 后接 `Linear(4,2)` 输出 logits。
- **约束与可选项**：`use_value_constraint` 时卷积/线性权重通过 `tanh` 映射到 `[-scale, scale]`；`use_tanh_activations` 可让中间激活保持在 `[-1,1]`。
