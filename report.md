...
## v3.1_debug：KD 设计与参数汇总
- **教师模型选择**：采用与学生同构的 Photonic MLP 学生网络作为 EMA 教师，初始权重拷贝学生，保证表征空间一致、蒸馏信号匹配且无需额外容量；教师 `eval()` 且关闭梯度，仅做前向，避免干扰学生训练。【F:train.py†L200-L235】
- **教师同构性确认**：代码在开启 KD 时直接调用 `build_student` 生成教师，并用学生最新权重初始化后全程冻结，只通过 EMA 平滑更新，确保教师始终是“学生的平滑版”而非更大网络，这与用户“不要被更大模型拉偏”需求一致。【F:train.py†L200-L338】
- **默认启动与自举**：运行 `train.py` 时默认启用 KD，脚本会自动用当前学生权重复制并冻结一份 EMA 教师，无需外部教师或额外训练流程；若需纯 CE，可用 `--no-enable_kd` 显式关闭。【F:train.py†L181-L239】【F:train.py†L304-L342】
- **为何不用更大教师**：
  - **数据与约束不支持大幅增容**：MIT-BIH 单导联样本量有限，且推理需保持 4 通道/轻量设计；引入更大/更深的教师（如 ResNet/Transformer）会在分布偏移下产生过度自信的软标签，之前尝试已观察到精度下降，违背“只带正向增益”的目标。
  - **特征空间对齐更重要**：同构教师与学生共享 photonic token 空间与归一化/约束逻辑，KD 不需要额外的投影或容量补偿即可传递有效梯度，避免因特征分布错位导致的误导。
  - **稳态与可复现性**：EMA 同构教师由学生权重平滑得到，天然贴合当前训练轨迹，软标签比外部大模型更平滑、更可控，降低 miss/FPR 摆动风险。
- **EMA 更新机制**：每步以 `kd_ema_decay=0.995` 对教师参数与 buffer 做滑动平均，同步学生最新分布但保持平滑，减少教师抖动对召回/误报的放大。【F:train.py†L329-L343】
- **KD 触发与强度**：默认开启 KD（如需关闭可显式 `--no-enable_kd`），`kd_start_epoch=5` 之后才加入 KD 损失，确保教师已稳定；`kd_alpha` 被裁剪到 [0,1]（默认 0.15），与 CE 按 `(1-α)*CE + α*KD` 混合，维持 CE 主导、防止精度被削弱。【F:train.py†L181-L239】【F:train.py†L308-L342】
- **温度与目标**：KD 仅对 logits 做软标签蒸馏，使用温度 `kd_temperature=2.5` 平滑教师分布，避免过置信导致学生过拟合；实现复用 `kd_logit_loss`（温度缩放 KL），与 EMA 教师搭配以轻量提升召回而不过度抬高 FPR。【F:train.py†L177-L235】【F:utils.py†L292-L297】
- **设计理由**：
  - **低侵入性**：虽然默认开启 KD，但系数被夹紧且延迟到第 5 轮才介入，只是对 CE 的温和补充，不会重塑训练动态；需要纯 CE 时可显式关闭。
  - **稳态教师**：EMA 平滑 + eval 冻结让教师输出更稳，减少早期噪声对召回/FPR 的负迁移，符合用户“对结果影响较小、只带来正向提升”的诉求。
  - **召回友好**：logit 级软目标在 miss 偏高时提供更平滑的异常置信度引导，配合现有阈值扫的召回增益，可在不显著推高 FPR 的前提下降低漏诊。

## v3.2_debug：默认 KD 导致 FPR 居高的排查与修正
- **问题根因**：默认开启 EMA KD 后，教师从第 1 轮开始跟随随机初始化，KD 权重也一次性全开，导致教师长期滞后学生；教师滞后时的蒸馏信号会把学生拉回旧分布，叠加轻度过采样后，决策边界过度偏向异常类，训练/验证阶段 FPR 长时间维持在 30%+。
  - **修正措施（已代码化）**：
    1) 推迟教师 EMA 更新到 `kd_start_epoch` 之后再同步，避免早期随机状态被教师“记住”；
    2) 新增 `--kd_ramp_epochs`，在启动 KD 后线性将 KD 权重从 0 拉升到 `kd_alpha`（默认 0.1），减轻突然加权带来的震荡与高 FPR；
    3) 新增教师质量守卫：仅当教师验证集 miss 不高于学生 +0.02、FPR 不高于学生 +0.03 时才继续使用 KD，否则当轮自动暂停 KD 并在日志中打印 `KD w=... (off: teacher lag)` 提示。
    4) 教师守卫的评估阈值与学生统一（采用当轮最佳阈值），避免教师用默认 0.5/argmax 计分被误判落后，导致 KD 长时间处于关闭状态。
- **预期影响**：KD 只在教师不落后学生且权重逐步拉升的情况下介入，减少高 FPR 与召回抖动；若教师持续落后，可直接 `--no-enable_kd` 做对照。
