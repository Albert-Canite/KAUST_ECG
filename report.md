...
- **KD 触发与强度**：默认关闭 KD（需 `--enable_kd` 开启），`kd_start_epoch=5` 之后才加入 KD 损失，确保教师已稳定；`kd_alpha` 被裁剪到 [0,1]（默认 0.15），与 CE 按 `(1-α)*CE + α*KD` 混合，维持 CE 主导、防止精度被削弱。【F:train.py†L177-L235】【F:train.py†L304-L338】
...
- **设计理由**：
  - **低侵入性**：默认关闭，参数夹紧且晚启用，确保即使开启也只是对 CE 的温和补充，不会重塑训练动态。
