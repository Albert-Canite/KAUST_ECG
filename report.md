...
## KD V1 DEBUG
- **用户现象**：教师在验证集指标优异，但在泛化集 miss≈9.8% 高于学生（miss≈8.1%），导致蒸馏后的学生 miss/FPR 未优于基线。
- **原因复盘**：教师对训练/验证分布过拟合，泛化召回低于学生，直接蒸馏会把学生拉向教师的偏置；同时 KD 仍强制使用 logits/feature 对齐，使学生放弃自身更好的泛化边界。
- **本轮修改**：
  - **教师质量守卫**：若教师在验证和泛化 miss 都劣于基线学生，则自动禁用 KD，将流程退化为仅用 CE 微调学生，避免“弱教师”反向指导。
  - **温和降权**：当教师仅表现出明显的泛化漏诊（miss gap > 5%）时，同时将 logits/feature 蒸馏权重减半，降低过拟合教师对学生的牵引力度。
  - **说明**：保留教师评估与 ROC 对比，但蒸馏是否启用由上述守卫决定，优先保证学生召回不被教师拖低。
